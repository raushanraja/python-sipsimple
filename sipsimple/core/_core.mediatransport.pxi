...
cdef class AudioTransport:
    ...
    def send_dtmf(self, digit):
        ...
        try:
            ...
            if self._obj == NULL:
                raise SIPCoreError("Stream is not active")
            if len(digit) != 1 or digit not in "0123456789*#ABCDR":
...
cdef class VideoTransport:
    ...
    def __init__(self, RTPTransport transport, VideoMixer video_mixer, BaseSDPSession remote_sdp=None, int sdp_index=0, list codecs=None):
        ...
        self._producer_slot = -1
        self._consumer_slot = -1
    ...
    property consumer_slot:

        def __get__(self):
            self._check_ua()
            if self._consumer_slot == -1:
                return None
            else:
                return self._consumer_slot

    property producer_slot:
    ...
    def stop(self):
        ...
        try:
            ...
            self._obj = NULL
            if self._consumer_slot != -1:
                self._video_mixer._remove_port(self._consumer_slot)
                self._consumer_slot = -1
            if self._producer_slot != -1:
                self._video_mixer._remove_port(self._producer_slot)
                self._producer_slot = -1
            '''
...
cdef class RttTextTransport:
    def __cinit__(self, *args, **kwargs):
        ...
        ua = _get_ua()
        pool_name = b"RttTextTransport_%d" % id(self)

        self.weakref = weakref.ref(self)
        Py_INCREF(self.weakref)

        status = pj_mutex_create_recursive(ua._pjsip_endpoint._pool, "rtt_text_transport_lock", &self._lock)
        if status != 0:
            raise PJSIPError("failed to create lock", status)
    ...
    def __init__(self, RTPTransport transport,
                 BaseSDPSession remote_sdp=None, int sdp_index=0):
        cdef int status
        cdef pj_pool_t *pool
        cdef pjmedia_endpt *media_endpoint
        cdef pjmedia_sdp_media *local_media_c
        ...
        if self.transport is not None:
            raise SIPCoreError("RttTextTransport.__init__() was already called")
        if transport is None:
            raise ValueError("transport argument cannot be None")
        if sdp_index < 0:
            raise ValueError("sdp_index argument cannot be negative")
